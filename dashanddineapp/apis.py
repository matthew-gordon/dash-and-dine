...
import stripe
from dashAndDine.settings import STRIPE_API_KEY
    ...
    if request.method == "POST":
        ...
        if len(order_details) > 0:
            ...
            charge = stripe.Charge.create(
                ...
                currency = "usd",
                source = stripe_token,
                description = "Dash and Dine Order"
            )
...
def driver_pick_up_order(request):
    ...
    if request.method == "POST":
        ...
        # Check if driver can only pick up on orderat the same time
        if Order.objects.filter(driver = driver).exclude(status = Order.ONTHEWAY):
            return JsonResponse({
                "status": "failed",
                "error": "You can only pick up one order at the same time."
            })
...
def driver_get_revenue(request):
    ...
    # Return difference between two days or times
